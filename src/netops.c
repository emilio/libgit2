	...
	const char *default_port)
{
	...
	const char *url_host, *url_port, *url_path, *url_userinfo;
	size_t url_host_len, url_port_len, url_path_len, url_userinfo_len;
	...
	url_host = url + u.field_data[UF_HOST].off;
	url_host_len = u.field_data[UF_HOST].len;

	has_port = !!(u.field_set & (1 << UF_PORT));
	url_port = url + u.field_data[UF_PORT].off;
	url_port_len = u.field_data[UF_PORT].len;

	has_path = !!(u.field_set & (1 << UF_PATH));
	url_path = url + u.field_data[UF_PATH].off;
	url_path_len = u.field_data[UF_PATH].len;
